...
**EEG Energy Optimizer** — a Home Assistant custom integration for grid-friendly battery management, optimized for energy communities (Energiegemeinschaften / EEG) in the DACH region. It controls when a PV battery charges and discharges to maximize feed-in during EEG-relevant time windows (mornings and evenings).
...
- **House Load Entry Guard**: The EEG benefit of a discharge is `discharge_power − house_load`. If the live house load exceeds the configured discharge power, the discharge is not started at all (`house_load_exceeds_discharge`) — all battery energy would go to the house, nothing reaches the community, and the difference is bought from the grid. Checked every cycle in `_check_common_guards`, blocks **entry only** (a running discharge is stopped by the watchdog below, so a load oscillating around the threshold cannot cycle the inverter). Fails open when `consumption_now_kw` is unavailable.
- **Grid Import Watchdog** (`_check_grid_import_watchdog`): Second line of defense once discharge is running (load can rise after entry). Grid import > 1 kW for > 5 consecutive minutes triggers an intervention whose severity depends on the inverter: **SolarEdge** aborts discharge for the rest of the day (`discharge_aborted_today`) because "Discharge to Maximize Export" never covers house load and the condition does not resolve itself; **all others** pause for `GRID_IMPORT_PAUSE_MINUTES` (15) and re-evaluate (`discharge_paused_grid_import`), since the usual cause is a transient load spike.
